...
playerTest.describe('Navigation (routes lazy)', () => {
  playerTest('la liste des groupes rend le groupe seedé @smoke', async ({ page }) => {
  ...
  playerTest('la page du groupe liste les fiches des personnages @smoke', async ({ page }) => {
    const s = seed();
  ...
  playerTest('la fiche de personnage charge sur l’onglet Survie @smoke', async ({ page }) => {
  ...
  playerTest('l’écran de connexion reste accessible @smoke', async ({ page }) => {
