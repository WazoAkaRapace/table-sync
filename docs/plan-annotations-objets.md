...
| **T** Écrire | tape = pose un point d'insertion → champ de saisie flottant au-dessus du clavier → Entrée valide, le texte se rend au point tapé (police italique existante — pas de police nouvelle) ; taille au **glisseur continu** (fraction de la largeur affichée, de ÷128 à ÷14 — l'aperçu et le composite sont strictement proportionnels), chaque note garde sa fraction |
| 📍 Tampons | galerie défilante de symboles SVG (`public/stamps/*.svg`, 1080×1080, teintes conçues — précache SW donc hors ligne) ; tape la carte = pose centrée au point tapé et le mode reste actif ; tape un tampon = sélection (anneau or) → le **glisseur de taille** le recadre EN DIRECT pendant le glissement (2 % à 20 % de la largeur) ; glissé = déplacement (même ancrage que les notes, centre borné à l'image) ; chaque tampon garde sa taille jusqu'au composite |
